...
function responseTo(){
	if(xmlHttp.readyState==4){
		...
		if((records.length/10)<=7){
			creatPageBar("RJ-pagebar", Math.ceil(records.length/10),lastindex,Math.ceil(records.length/10),show_page);
		}else{
			creatPageBar("RJ-pagebar", records.length/10,lastindex,7,show_page);
		}
		adjustDis();
