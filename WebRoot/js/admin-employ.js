...
function responseTo1(){
	if(xmlHttp.readyState==4){
		...
		total = Math.ceil(enterprises.length/10);		//计算分页的总页数
		...
		for(var tempindex=0; tempindex<total; tempindex++){
			...
			for(var i=0;i<10;i++){
				if(10*tempindex+i>=enterprises.length)break;	//此时说明所有图片都添加完毕
				html += "<tr><td width=\"400px\"><a href=\"javascript:list_aem(\'"+enterprises[10*tempindex+i]+"\')\" class=\"ename\">"+enterprises[10*tempindex+i].split("@")[0]+"</a></td><td width=\"400px\"><a class=\"renum\">"+num[10*tempindex+i]+"个职位</a></td></tr>";
			}
			html += "</tbody>";
			html += "</table>";
			html+='</div>';
		...
		if((enterprises.length/10)<=7){
			creatPageBar("RJ-pagebar1", Math.ceil(enterprises.length/10),lastindex,Math.ceil(enterprises.length/10),show_page);
		}else{
			creatPageBar("RJ-pagebar1", enterprises.length/10,lastindex,7,show_page);
		}
		adjustDis(1);
	}
}
function responseTo2(){
	if(xmlHttp.readyState==4){
		...
		for(var tempindex=0; tempindex<total; tempindex++){
			...
			for(var i=0;i<10;i++){
				if(10*tempindex+i>=enterprises.length)break;	//此时说明所有图片都添加完毕
				html += "<tr><td width=\"400px\"><a href=\"javascript:list_nem(\'"+enterprises[10*tempindex+i]+"\')\" class=\"ename\">"+enterprises[10*tempindex+i].split("@")[0]+"</a></td><td width=\"400px\"><a class=\"renum\">"+num[10*tempindex+i]+"个职位</a></td><tr>";			
			}
			html += "</tbody>";
			html += "</table>";
			html+='</div>';
		...
		//在id为RJ-pagebar的div中创建页码条，总页数不大于7的时候显示页数和总页数相等，大于7时显示页数为7
		if((enterprises.length/10)<=7){
			creatPageBar("RJ-pagebar2", Math.ceil(enterprises.length/10),lastindex,Math.ceil(enterprises.length/10),show_page_);
		}
		else{
			creatPageBar("RJ-pagebar2", enterprises.length/10,lastindex,7,show_page_);
		}
		adjustDis(2);
	}
}
