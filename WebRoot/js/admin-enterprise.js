...
function responseTo1(){
		...
		if((enterprises.length/10)<=7){
			creatPageBar("RJ-pagebar", Math.ceil(enterprises.length/10),lastindex,Math.ceil(enterprises.length/10),show_page);
		}else{
			creatPageBar("RJ-pagebar", enterprises.length/10,lastindex,7,show_page);
		}
		adjustDis();
	}
}
